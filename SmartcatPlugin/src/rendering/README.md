...
test/preview at [www.smartcatplugin.localhost](https://www.smartcatplugin.localhost).
Be sure to watch the `rendering` container logs for compilation errors. To debug, you
can attach to the `SmartcatPlugin.exe` process inside the `rendering` container.
